...
In order to implement iptables rules we also need the flag `--cap-add=NET_ADMIN`, we use this to minimize the attack surface instead of using e.g `--privileged`.
...
[+] Building 2.3s (14/14) FINISHED                                                                                                              docker:default
 ...
 => [1/9] FROM docker.io/library/ubuntu:latest@sha256:1e622c5f073b4f6bfad6632f2616c7f59ef256e96fe78bf6a595d1dc4376ac02                                    0.0s
 => [internal] load build context                                                                                                                         0.0s
 => => transferring context: 1.39kB                                                                                                                       0.0s
 => CACHED [2/9] RUN apt-get update &&     apt-get install -y python3 python3-pip python3-venv dnsutils curl base58 iptables &&     apt-get clean && rm   0.0s
 => CACHED [3/9] WORKDIR /app                                                                                                                             0.0s
 => CACHED [4/9] COPY requirements.txt /app/                                                                                                              0.0s
 => CACHED [5/9] RUN python3 -m venv /app/venv                                                                                                            0.0s
 => CACHED [6/9] RUN ./venv/bin/pip install --no-cache-dir -r requirements.txt                                                                            0.0s
 => [7/9] COPY . /app/                                                                                                                                    0.0s
 => [8/9] RUN chown -R www-data:www-data /app                                                                                                             1.0s
 => [9/9] RUN chmod +x iptables-rules.sh                                                                                                                  0.2s
 => exporting to image                                                                                                                                    0.1s
 => => exporting layers                                                                                                                                   0.1s
 => => writing image sha256:cd3cbda00f1af0abe40a8443e820aa06f229e228db6504554ed30da0553382a0                                                              0.0s
 => => naming to docker.io/library/dns-exfil                                                                                                              0.0s

» docker run -d --name dns-exfil --cap-add=NET_ADMIN --add-host attacker.com:172.17.0.1 -p 8081:80 dns-exfil
87e3168ad02ca27066a8aab9dcdc3a28fde90d444e92080cbf4d15acd97c7851
...
» docker exec -it dns-exfil /bin/bash                                                                       
